...
function AssistantRecommend() {
  const { addFavorite, removeFavorite, assistants } = useAssistantStore()
  ...
  const initAssistantMarket = useCallback((refresh: boolean = false) => {
    ...
    if (refresh || recommendation.length === 0) {
      // For now, just recommend the default assistant
      recommend(1)
    }
  }, [])
  ...
  const handleSelectAssistant = useCallback(
    async (identifier: string) => {
      ...
      const assistant = assistants.find(a => a.identifier === identifier)
    ...
    <div className="flex w-full flex-1 grow items-center justify-center overflow-y-auto scroll-smooth p-4 text-sm">
      <section className="-mt-20 w-full max-sm:mt-0">
        <div className="my-3 flex justify-between">
          <h3 className="text-base font-medium">assistantRecommend</h3>
          <div>
            <Button
              className="h-6 w-6 p-1"
              title="customizeAssistantSetting"
              variant="ghost"
              size="icon"
